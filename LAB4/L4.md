...
```
![case_type_stats](./images/1_case_type_stats.png)
...
```
![staff_autopsy_count](./images/2_staff_autopsy_count.png)
...
```
![staff_autopsy_dates](./images/3_staff_autopsy_dates.png)
...
```
![analysis_by_case_type](./images/4_analysis_by_case_type.png)
...
```
![arrival_and_autopsy](./images/1_arrival_and_autopsy.png)
...
```
![staff_analysis_details](./images/2_staff_analysis_details.png)
...
```
![full_analysis_info](./images/3_full_analysis_info.png)
...
FROM deceased;
```
![analysis_count_per_deceased](./images/1_analysis_count_per_deceased.png)
...
У вкладеному SELECT стоїть COUNT(*) і перевірка
WHERE a.deceased_id = deceased.deceased_id тобто для кожного конкретного померлого рахується кількість аналізів, які з ним пов’язані через таблицю autopsy.
...
```
![top_staff_by_analysis](./images/2_top_staff_by_analysis.png)
Запит шукає працівників, які зробили найбільше аналізів. Спочатку в підзапиті ```sql SELECT COUNT(*) FROM laboratory_analysis WHERE la.staff_id = s.staff_id ``` для кожного працівника рахується кількість його аналізів.
Далі ще один вкладений запит знаходить максимум серед усіх цих значень: ```sql SELECT MAX(cnt) FROM (SELECT COUNT(*) AS cnt ... ) sub``` 
в основному WHERE йде порівняння, якщо кількість аналізів працівника дорівнює максимуму, він потрапляє у результат.
...
```
![toxicology_cases](./images/3_toxicology_cases.png)
Цей запит відбирає тільки тих померлих, у яких був токсикологічний аналіз. У підзапиті:
```sql WHERE la.test_type = 'токсикологічний'``` ми отримуємо всі deceased_id, для яких такий аналіз існує.
А вже в основному запиті використовується: ```WHERE d.deceased_id IN ()``` тобто перевіряється, чи входить померлий у цей список.
